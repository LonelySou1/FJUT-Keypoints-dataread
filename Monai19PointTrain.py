...
import math
import matplotlib.pyplot as plt
from model import UNetHao3,UNetEGA3
# 参与训练
result = [] # 存储验证结果
def show_masked_image(x, points, additional_points=None):
    # 将张量从 GPU 移动到 CPU，并将其转换为 NumPy 数组
    x_np = x.cpu().detach().numpy()

    # 如果 x 的形状是 (B, C, H, W)，你需要选择一个批次和通道
    # 例如，我们选择第一个批次和第一个通道
    batch_index = 0
    channel_index = 0
    x_np = x_np[batch_index, channel_index, :, :]

    # 绘制图像
    plt.imshow(x_np, cmap='gray')  # 使用灰度图显示
    plt.title('Masked Image')
    plt.axis('off')  # 关闭坐标轴显示

    # 获取 points 的坐标
    points_np = points.cpu().detach().numpy()
    points_np = points_np[0]  # 假设 points 的形状是 (1, 29, 2)

    # 在图像上标注点
    plt.scatter(points_np[:, 0], points_np[:, 1], c='red', s=15)  # 使用红色标注点，大小为10

    # 如果提供了 additional_points，则绘制这些点
    if additional_points is not None:
        additional_points_np = additional_points.cpu().detach().numpy()
        additional_points_np = additional_points_np[0]  # 假设 additional_points 的形状是 (1, 29, 2)
        plt.scatter(additional_points_np[:, 0], additional_points_np[:, 1], c='blue', s=5)  # 使用蓝色标注点，大小为10
        # print('pre:', points.to(device), 'label:',additional_points.to(device))
        # print(points.to(device)-additional_points.to(device))
    # 显示图像
    plt.show()
...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    for i, item in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            # if i==3:
            #     print(points)
            #     show_masked_image(images, points)
...
def main(args): # 传递配置文件
    ...
    tdata_dir = r"C:\Users\18066\Desktop\Key_or\data\train"
    data_dir = r"C:\Users\18066\Desktop\Key_or\data\test1"
    ...
    train_transforms = Compose([
        ...
        ReadPoint19point(size=672),
    ])
    cache_dir1 = "/data/hliang/train"
    cache_dir2 = "/data/hliang/test"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # images = torch.stack([image.to(device) for image in images]): 将图像数据转移到设备（如GPU）并堆叠
            ...
            y = torch.tensor([1935 / 6720,2400 / 6720]).to(device) # 与关键点热力图对应，但是要多加个0，因为1935X2400是0.1毫米
        ...
        # 保存的文件名包含当前的 SDR 值
        torch.save(save_files, "/home/hliang/Key/weight/55model-{}.pth".format(l.item() / 150.0))
