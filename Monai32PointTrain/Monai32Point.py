...
from MyTransform import ReadPoint32point
...
def main(args): # 传递配置文件
    ...
    tdata_dir = "/home/hliang/32points/data1/train"
    data_dir = "/home/hliang/32points/data1/test"
    ...
    t_labels = sorted([os.path.join(tdata_dir, f) for f in os.listdir(tdata_dir) if f.endswith(".json")])
    ...
    labels = sorted([os.path.join(data_dir, f) for f in os.listdir(data_dir) if f.endswith(".json")])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # images = torch.stack([image.to(device) for image in images]): 将图像数据转移到设备（如GPU）并堆叠
            ...
            y = torch.tensor([2300 / 6720,1900 / 6720]).to(device) # 与关键点热力图对应，但是要多加个0，因为1935X2400是0.1毫米
        ...
        # 保存的文件名包含当前的 SDR 值
        torch.save(save_files, "/home/hliang/32points/weight/55model-{}.pth".format(l.item() / 150.0))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume', default='/home/hliang/32points/weight/55model-1.2965850830078125.pth', type=str, help='resume from checkpoint')
    # 指定接着从哪个epoch数开始训练
